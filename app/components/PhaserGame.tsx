...
export default function PhaserGame({
  ...
  useEffect(() => {
    ...
    const setChimpPointsRef = ((window as any).__SET_CHIMP_POINTS__ =
      setChimpPoints);
    class MainScene extends Phaser.Scene {
      ...
      createBackground() {
        ...
        if (tileTexture && typeof (tileTexture as any).refresh === "function") {
      ...
      replaceChimp(key: string) {
        ...
        if (!this.anims.exists(restKey)) {
          this.anims.create({
            key: restKey,
            frames: this.anims.generateFrameNumbers(key, { start: 6, end: 13 }),
            frameRate: 6,
            repeat: -1,
          });
        }
        ...
        if (this.collectible && this.chimp) {
          ...
          if (distance < minDistance) {
            ...
            if (typeof (window as any).__SET_CHIMP_POINTS__ === "function") {
              (window as any).__SET_CHIMP_POINTS__((prev: number) => prev + 1);
            }
          }
        }
      }
    ...
    if (container && !(window as any).__PHASER_GAME__) {
      const game = new Phaser.Game({
        ...
        parent: container, // Attach canvas here
        scale: {
