...
import { AiOutlineCopy, AiOutlineDownload, AiOutlineLink } from "react-icons/ai";
...
export default function Home() {
  ...
  const parseUrlParams = useCallback(() => {
    ...
    if (presetParam) {
      const presetIndex = reactionsMap.findIndex(r => r.title.toLowerCase() === presetParam.toLowerCase());
      if (presetIndex >= 0) {
        setOverlayNumber(presetIndex + 1);
      }
    } else {
      // Default to CHIMP preset if not specified
      const chimpIndex = reactionsMap.findIndex(r => r.title.toLowerCase().includes('chimp'));
      if (chimpIndex >= 0) {
    ...
    if (collectionParam) {
      const collectionIdx = collectionsMetadata.findIndex(c => 
    ...
    if (watermarkParam !== null) {
      setOverlayEnabled(watermarkParam.toLowerCase() === 'true' || watermarkParam === 'made with chimp.fun');
    }
