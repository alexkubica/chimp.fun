...
export const ReactionOverlayDraggable = memo(function ReactionOverlayDraggable({
  x,
  y,
  scale,
  imageUrl,
  ...
  const onMouseDown = useCallback((e: MouseEvent<HTMLDivElement>) => {
    ...
    e.stopPropagation();
    e.preventDefault();
  }, [x, y, scale, containerSize, naturalSize, setDragging]);
  ...
  const onMouseMove = useCallback((e: MouseEvent | globalThis.MouseEvent) => {
    if (dragging) {
    ...
    if (resizing && naturalSize) {
      ...
      let newOverlayHeight = (start.naturalHeight / start.naturalWidth) * newOverlayWidth;
      
      const minSize = 50;
      if (newOverlayWidth < minSize || newOverlayHeight < minSize) {
        if (newOverlayWidth < newOverlayHeight) {
          newOverlayWidth = minSize;
          newOverlayHeight = (start.naturalHeight / start.naturalWidth) * minSize;
        } else {
          newOverlayHeight = minSize;
          newOverlayWidth = (start.naturalWidth / start.naturalHeight) * minSize;
        }
      }
  ...
  }, [dragging, resizing, start, containerSize, naturalSize, scale, x, y, onChange]);

  const onTouchMove = useCallback((e: TouchEvent | globalThis.TouchEvent) => {
  ...
  }, [setDragging, setResizing, onDragEnd, resizing, onResizeEnd]);
  ...
  const onResizeMouseDown = useCallback((e: MouseEvent<HTMLDivElement>) => {
    ...
    e.stopPropagation();
  }, [scale, containerSize, naturalSize, setResizing]);

  const onResizeTouchStart = useCallback((e: React.TouchEvent<HTMLDivElement>) => {
    setResizing(true);
    const touch = e.touches[0];
    setStart((prev) => ({
      ...prev,
  ...
  }, [dragging, resizing, onMouseMove, onMouseUp, onTouchMove, onTouchEnd]);
