...
export async function POST(request: NextRequest) {
  try {
    ...
    const watermarkScale = parseFloat(formData.get('watermarkScale') as string) || 1;
    const watermarkPaddingX = parseInt(formData.get('watermarkPaddingX') as string) || 20;
    const watermarkPaddingY = parseInt(formData.get('watermarkPaddingY') as string) || 20;
    ...
    const tempDir = join(tmpdir(), `ffmpeg-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
    ...
    try {
      ...
      await writeFile(inputImagePath, Buffer.from(await inputImageFile.arrayBuffer()));
      await writeFile(reactionImagePath, Buffer.from(await reactionImageFile.arrayBuffer()));
      ...
      if (overlayEnabled && watermarkImageFile) {
        ...
        await writeFile(watermarkImagePath, Buffer.from(await watermarkImageFile.arrayBuffer()));
      ...
      await Promise.all([
        ...
        watermarkImageFile ? unlink(join(tempDir, 'watermark.png')).catch(() => {}) : Promise.resolve(),
