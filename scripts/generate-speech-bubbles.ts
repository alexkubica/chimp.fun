...
async function generateSpeechBubbles() {
  ...
  for (const text of speechTexts) {
    try {
      ...
      await page.goto('https://pixelspeechbubble.com/', { waitUntil: 'networkidle2' });
      
      // Debug: Check what elements are on the page
      const pageContent = await page.evaluate(() => {
        return {
          textareas: Array.from(document.querySelectorAll('textarea')).map(t => t.tagName),
          inputs: Array.from(document.querySelectorAll('input')).map(i => ({ tag: i.tagName, type: i.type, value: i.value })),
          buttons: Array.from(document.querySelectorAll('button')).map(b => ({ text: b.textContent?.trim(), type: b.type })),
          allButtons: Array.from(document.querySelectorAll('input[type="button"], input[type="submit"], button')).map(b => ({ 
      ...
      let createButton = await page.$('input[type="button"][value="Create"]') as any;
      if (!createButton) {
      ...
      try {
        ...
        const alternativeSelectors = ['img', 'canvas', '[src*="data:image"]', 'img[src*="blob:"]'];
      ...
      if (imageSrc && imageSrc !== 'about:blank') {
        ...
        try {
          ...
          if (imageElement) {
            const filename = createFilename(text);
            const filePath = path.join(downloadPath, filename) as `${string}.png`;
...
async function updateReactionsMap(newReactions: any[]) {
  ...
  if (match) {
    ...
    const formattedReactions = newReactions.map(reaction => {
      return `  {
...
${match[1]}];${match[2]}`;
    ...
    console.error('❌ Could not find reactionsMap array end in consts/index.ts');
  }
}
